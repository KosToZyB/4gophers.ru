+++
date = "2015-06-28T22:37:06+03:00"
draft = false
title = "Анонимизация изображение с помощью Go"
...
В стандартной библиотеке Go есть достаточное количество мощных инструментов для работы с изображениями. Это пакетв `image`, `image/*` и `draw`. В этом руководстве мы будем использовать эти инструменты совместно с [Machine Box Go SDK](https://github.com/machinebox/sdk-go) для цензурирования изображений.

Мы напишем простую консольную утилиту, но тот же самый код модно будет использовать, например, для написания веб-приложения.

## Запускаем Facebox

Facebox это один из образов [Machine Box](https://machinebox.io/) который позволяет распознавать лица на основе машинного обучения. Все это поставляется в виде Docker контейнера, который можно запускать где угодно, в том числе и локально.
...
Обратите внимание, что вам обязательно нужно указать переменную окружения `MB_KEY`. Значение для этой переменной можно найти на странице вашего аккаунта [https://machinebox.io/account](https://machinebox.io/account). Естественно, вам необходимо зарегистрироваться на machinebox.io.

Как только контейнер запуститься, можно пройти по ссылке `http://localhost:8080/` для доступа к интерактивной консоли встроенной в Facebox.

Facebox можно обучить распознавать какие-то конкретные лица и затем пользоваться им для поиска похожих лиц. Но в нашем примере не будет использоваться распознавание лиц, достаточно получать информацию о том, где на фотографии расположено лицо.
...
Кроме импорта пакетов ничего больше не нужно делать. Каждый пакет в функции `init` регистрирует функцию `image.Decode`. Но в Go нельзя просто так импортировать пакет и не использовать его. Чтобы наша программа компилировалась, используем небольшой хак - укажем в качестве алиасов символ подчеркивания. 

В результате секция с импортируемыми пакетами будет выглядеть прримерно так:
...
* Можно зафоловить [@matryer](https://twitter.com/matryer) в твиттере и следить за новыми статьями. И не забывайте про [@machineboxio](https://twitter.com/machineboxio)